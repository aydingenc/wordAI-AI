...
export interface QuizScreenProps {
  ...
  storyTitle: string;
  onFinish: (result: { correctCount: number; totalQuestions: number; xpEarned: number; learnedWords: string[] }) => void;
  onBack: () => void;
  onClose: () => void;
...
const WORD_QUIZ_BANK: Record<string, WordBankEntry> = {
  adventure: {
    sentence: 'Every step felt like the start of a new ___.',
    choices: ['adventure', 'fear', 'tiredness', 'boredom'],
    correctIndex: 0,
    hint: 'Bu kelime, yeni ve bilinmeyen bir şeye atılan, heyecanlı ve biraz riskli bir deneyimi anlatır.',
  },
  journey: {
    sentence: 'It was going to be a long ___.',
    choices: ['hotel', 'journey', 'ticket', 'airport'],
    correctIndex: 1,
    hint: 'Bu kelime, bir yerden başka bir yere yapılan, genellikle uzun süren bir seyahati anlatır.',
  },
  luggage: {
    sentence: 'Carrying her ___ became harder every hour.',
    choices: ['luggage', 'document', 'food', 'map'],
    correctIndex: 0,
    hint: 'Bu kelime, seyahat ederken yanında taşıdığın valiz/bavul gibi eşyaları anlatır.',
  },
  passport: {
    sentence: 'She had to show her ___ at the counter.',
    choices: ['credit card', 'ticket', 'passport', 'key'],
    correctIndex: 2,
    hint: 'Bu kelime, yurt dışına çıkarken kimliğini kanıtlamak için gösterdiğin resmi belgedir.',
  },
  gate: {
    sentence: 'He checked the ___ number one more time.',
    choices: ['gate', 'carousel', 'counter', 'lounge'],
    correctIndex: 0,
    hint: 'Bu kelime, havalimanında uçağa binerken geçtiğin, üzerinde numara yazan kapıyı anlatır.',
  },
  love: {
    sentence: 'She said she would always ___ learning new things.',
    choices: ['love', 'forget', 'avoid', 'fear'],
    correctIndex: 0,
    hint: 'Bu kelime, birine ya da bir şeye karşı duyulan çok güçlü ve sıcak bir bağlılık duygusunu anlatır.',
  },
  travel: {
    sentence: 'They wanted to ___ to a new country every year.',
    choices: ['travel', 'stay', 'sleep', 'wait'],
    correctIndex: 0,
    hint: 'Bu kelime, bir yerden başka, genellikle uzak bir yere gitmeyi anlatır.',
  },
  airport: {
    sentence: 'The ___ was crowded this morning.',
    choices: ['airport', 'museum', 'library', 'market'],
    correctIndex: 0,
    hint: 'Bu kelime, uçakların inip kalktığı, check-in ve pasaport kontrolünün yapıldığı yeri anlatır.',
  },
  coffee: {
    sentence: 'She drinks ___ without sugar.',
    choices: ['coffee', 'water', 'soup', 'juice'],
    correctIndex: 0,
    hint: 'Bu kelime, kavrulmuş çekirdeklerden yapılan, sıcak içilen, uyandırıcı bir içeceği anlatır.',
  },
  mountain: {
    sentence: 'They climbed the tall ___.',
    choices: ['mountain', 'building', 'ladder', 'hill'],
    correctIndex: 0,
    hint: 'Bu kelime, yerden çok yükseğe uzanan, genellikle zirvesine tırmanılan doğal bir oluşumu anlatır.',
  },
  sunset: {
    sentence: 'We watched the ___ by the sea.',
    choices: ['sunset', 'sunrise', 'storm', 'moonlight'],
    correctIndex: 0,
    hint: 'Bu kelime, güneşin akşam ufukta kaybolurken gökyüzünü renklendirdiği anı anlatır.',
  },
  dream: {
    sentence: 'He follows his biggest ___.',
    choices: ['dream', 'fear', 'job', 'rule'],
    correctIndex: 0,
    hint: 'Bu kelime, uyurken görülen ya da gelecekte gerçekleşmesi çok istenen bir şeyi anlatır.',
  },
  ocean: {
    sentence: 'The ___ looked calm and blue.',
    choices: ['ocean', 'desert', 'valley', 'forest'],
    correctIndex: 0,
    hint: 'Bu kelime, dünyanın büyük bir bölümünü kaplayan, uçsuz bucaksız tuzlu su kütlesini anlatır.',
  },
  city: {
    sentence: 'The ___ never sleeps at night.',
    choices: ['city', 'village', 'farm', 'island'],
    correctIndex: 0,
    hint: 'Bu kelime, çok sayıda insanın, binanın ve caddenin bulunduğu büyük bir yerleşim yerini anlatır.',
  },
};

const FALLBACK_DISTRACTORS = ['ticket', 'flight', 'hotel', 'map', 'key', 'forest', 'music', 'friend'];

const COMPREHENSION_QUESTIONS: Omit<ComprehensionQuestion, 'kind' | 'kicker'>[] = [
  {
    text: 'When did the story begin?',
    choices: ['In the early morning', 'Late at night', 'In the afternoon', 'At midnight'],
    correctIndex: 0,
    hint: 'İpucu: Hikâyenin ilk cümlesinde günün hangi saatinden bahsedildiğine bak.',
  },
  {
    text: 'How did the story end, according to the text?',
    choices: ['With memories forming from what they learned', 'With them getting lost forever', 'With an argument', 'With them going home immediately'],
    correctIndex: 0,
    hint: 'İpucu: Hikâyenin son cümlesinde neyin yavaşça neye dönüştüğüne bak.',
  },
  {
    text: 'What kind of feeling does the story describe about the discovery along the way?',
    choices: ['Quiet and bright', 'Loud and scary', 'Sad and dark', 'Boring and slow'],
    correctIndex: 0,
    hint: 'İpucu: İkinci paragrafta o anın nasıl hissettirdiği iki sıfatla anlatılıyor.',
  },
];
...
function buildWordQuestion(word: string): Omit<WordQuestion, 'kind' | 'kicker'> {
  ...
  return {
    ...
    hint: 'İpucu: Bu kelimenin anlamını, hikâyedeki cümlenin bağlamından çıkarmaya çalış.',
    word: key,
  };
}

function buildQuestions(targetWords: QuizTargetWord[]): Question[] {
  const total = targetWords.length + COMPREHENSION_QUESTIONS.length;
  const wordQuestions: WordQuestion[] = targetWords.map((tw, i) => ({
    kind: 'word',
    kicker: `KELİME · ${i + 1}/${total}`,
    ...buildWordQuestion(tw.word),
  }));
  const comprehensionQuestions: ComprehensionQuestion[] = COMPREHENSION_QUESTIONS.map((c, i) => ({
    kind: 'comprehension',
...
function getResultTier(pct: number): ResultTier {
  if (pct < 0.4) {
    return {
      ...
      title: 'Yeterli değil',
      sub: 'Hikâye’ye tekrar dön, bol bol oku ve dinle.',
...
export function QuizScreen({
  ...
  const questions = useMemo(() => buildQuestions(targetWords), [targetWords]);
  ...
  if (showResults) {
    return (
      <ResultsScreen
        ...
        onNewStorySameWords={onNewStorySameWords}
...
function ResultsScreen({
  ...
  onBack: () => void;
  onFinish: QuizScreenProps['onFinish'];
  onClose: () => void;
  onDifferentTheme: () => void;
  onPracticeWords: () => void;
  onNewStorySameWords: () => void;
  ...
  const handleCta = () => {
    ...
    onFinish({
      ...
      xpEarned: xp,
      learnedWords: targetWords.filter((tw) => wordResults[tw.word]).map((tw) => tw.word),
    });
  };
  ...
  return (
    <View style={styles.resultsRoot}>
      ...
      <ScrollView
        ...
        contentContainerStyle={[styles.resultsScrollContent, { paddingTop: insetsTop + 44, paddingBottom: insetsBottom + 24 }]}
        ...
        {isLowTier ? (
          <>
            <Pressable onPress={onDifferentTheme}>
              <LinearGradient colors={[TOKENS.violet400, TOKENS.violet600]} start={{ x: 0, y: 0 }} end={{ x: 1, y: 1 }} style={styles.ctaSecondaryFilled}>
                <Text style={styles.ctaSecondaryFilledText}>Farklı Tema Farklı Hikâyeler</Text>
              </LinearGradient>
            </Pressable>
...
const styles = StyleSheet.create({
  ...
  closeBtn: { position: 'absolute', top: 28, right: 26, zIndex: 10, width: 38, height: 38, borderRadius: 12, backgroundColor: 'rgba(255,255,255,0.06)', borderWidth: 1, borderColor: 'rgba(255,255,255,0.12)', alignItems: 'center', justifyContent: 'center' },
  resultsScroll: { flex: 1 },
  resultsScrollContent: { paddingHorizontal: 20, alignItems: 'center', flexGrow: 1 },
  ...
  ctaSecondaryFilled: { padding: 14, borderRadius: 15, alignItems: 'center' },
  ctaSecondaryFilledText: { fontFamily: 'Inter_700Bold', fontSize: 14, color: '#FFFFFF' },
